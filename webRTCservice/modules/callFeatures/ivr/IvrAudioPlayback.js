...
function createIvrAudioPlayback({
    ...
    function ensurePlaybackReady(sessionId, session, stateLabel, state) {
        ...
        if (!(typeof isInCall === "function" && isInCall(session))) {
    ...
    function getIvrOutput(session) {
        return session?.resources?.mediaSession?.().getLeg("ivr") || session?.media?.legs?.ivr || null;
    }

    function hasIvrOutput(session) {
        return Boolean(getIvrOutput(session));
    }

    function isMediaActive(session) {
        const graph = session?.resources?.mediaSession?.().getGraph?.();
        return Boolean(graph?.active || session?.mediaRelayActive || session?.ivr?.active);
    }

    function writeIvrRtp(session, packet) {
        const ivrLeg = getIvrOutput(session);
        if (ivrLeg) {
            ivrLeg.emitRtp(packet);
        }
    }

    function notifyIvrSource(session, packet) {
    ...
    async function playText(sessionId, text, { interrupt = true, waitForCompletion = false } = {}) {
        ...
        const session = sessions.get(sessionId);
        if (!hasIvrOutput(session)) {
            logger.warn(`[${sessionId}] IVR audio skipped: media output missing`);
            return false;
        }
        ...
        let idx = 0;
        notifyIvrSource(session, { header: timeline });
        state.timer = setInterval(() => {
            if (state.stopped || !isMediaActive(session)) return;
    ...
    async function playFile(sessionId, fileNameOrPath, { interrupt = true, waitForCompletion = false, loop = false } = {}) {
        ...
        const session = sessions.get(sessionId);
        if (!hasIvrOutput(session)) {
            logger.warn(`[${sessionId}] IVR audio file skipped: media output missing`);
            return false;
        }
