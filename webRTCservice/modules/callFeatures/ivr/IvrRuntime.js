...
function createIvrRuntime({
    ...
    startMediaForSession = null,
    stopMediaForSession = null,
    ...
    async function startIvr(sessionId, meta = {}) {
        ...
        session.resources?.register("ivr", (reason = "ivr-resource-stop") => stopIvr(sessionId, reason));
        ...
        if (typeof startMediaForSession === "function") {
            await startMediaForSession(sessionId, meta);
        }
    ...
    function stopIvr(sessionId, reason = "call-ended") {
        ...
        session.resources?.remove?.("ivr");
        if (typeof stopAudioForSession === "function") {
            stopAudioForSession(sessionId, `ivr-stop:${reason}`).catch(() => {});
        }
        if (typeof stopMediaForSession === "function") {
            stopMediaForSession(sessionId, `ivr-stop:${reason}`).catch(() => {});
        }
        sendDataChannelMessage(sessionId, {
            msgType: "call",
            action: "ivr-state",
...
async function onDigit3(ctx) {
    ...
    await ctx.say("redirecting to 9 7 2 5 5 7 0 1 2 4 0 7", "ivr-digit-3-redirect", extra);
